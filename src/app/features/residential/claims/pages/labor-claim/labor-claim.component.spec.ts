...
describe("LaborClaimComponent", () => {
  ...
  it("checkClaimType maps claimDetails.claimType to internal path and sets flags", () => {
    const cases: Array<{ input: string; expected: string }> = [
      ...
      { input: "Assurance Warranty Claim", expected: CLAIM_PATH_NAMES.CUSTOMER_SATISFACTION },
      { input: "Order Error Claim", expected: CLAIM_PATH_NAMES.MOHAWK_ORDER_ERROR },
    ];
  ...
  it("checkClaimType covers additional mappings (tax, defective, wrong, damage, quantity, cancellation)", () => {
    const more: Array<{ input: string; expected: string }> = [
      ...
      { input: 'Accommodation Return', expected: CLAIM_PATH_NAMES.ACCOMMODATION_RETURN },
      { input: 'Defective Product Claim', expected: CLAIM_PATH_NAMES.DEFECTIVE_PRODUCT },
      { input: 'Wrong Product Claim', expected: CLAIM_PATH_NAMES.WRONG_PRODUCT },
      { input: 'Damage Claim', expected: CLAIM_PATH_NAMES.DAMAGED },
      { input: 'Quantity Claim', expected: CLAIM_PATH_NAMES.WRONG_QUANTITY_SHORTAGE },
      { input: 'Cancellation Fees', expected: CLAIM_PATH_NAMES.CANCELLATION_FEE },
    ];
  ...
  it("goToConfirmation should navigate, reset form and clear selectedInvoiceLines when isNewClaim true", () => {
    ...
    (component as any).router = { navigate: jasmine.createSpy('navigate') } as any;
    ...
    expect((component as any).router.navigate).toHaveBeenCalledWith([
      `/residential/claims/wrong-product-claim/confirmation`
  ...
  it("continue should set spinnerLoading false on error", fakeAsync(() => {
    ...
    spyOn(mockClaims, "getClaimsDetails").and.returnValue(throwError(() => new Error("fail")));
  ...
  it("scrollToInvalidControl should search inside scrollTarget when not on document", () => {
    ...
    component.scrollTarget = { nativeElement: { querySelector: jasmine.createSpy("q").and.returnValue(el) } } as any;

    const spyScroll = spyOn(el, "scrollIntoView");
    component.scrollToInvalidControl("ctrl");
    expect((component.scrollTarget.nativeElement.querySelector as jasmine.Spy)).toHaveBeenCalled();
    expect(spyScroll).toHaveBeenCalled();
    expect((el as any).focus).toHaveBeenCalled();
  });
  ...
  it("scrollToInvalidControl retries until element appears", fakeAsync(() => {
    ...
    component.scrollTarget = { nativeElement: { querySelector: jasmine.createSpy("q").and.returnValue(null) } } as any;
  ...
  it("removeInvoice should clear selectedInvoiceLines.line", () => {
    mockClaims.selectedInvoiceLines = { line: [1, 2, 3], invoiceNumber: "INV1" } as any;
  ...
  it("getUserDetails should call getCurrentUserDetail and not call progressHide on success", () => {
    ...
    spyOn(userService, 'getCurrentUserDetail').and.returnValue(of({ body: { name: 'u' } }));
  ...
  it("getUserDetails should call progressHide on error", () => {
    const userService = TestBed.inject(UserService) as any;
    spyOn(userService, 'getCurrentUserDetail').and.returnValue(throwError(() => new Error('fail')));
    spyOn(userService, 'progressHide');
  ...
  it("saveForm should call updateClaim and stopAlert(false) on successful update", fakeAsync(() => {
    ...
    component.ngOnInit();
    component.newClaimForm.controls['replacedWithMohawkMaterial'].setValue(true);
    ...
    component.child = { onControlChange: jasmine.createSpy('onControlChange') } as any;
    component.newchild = { claimsService: { selectedInvoiceLines: { line: [], invoiceNumber: false } } } as any;
    ...
    spyOn(mockClaims, 'updateClaim').and.returnValue(of({ body: { claimNumber: 'CN1' } }));
  ...
  it("saveForm should call createLaborClaim and stopAlert(true) on successful create", fakeAsync(() => {
    ...
    spyOn(mockClaims, 'createLaborClaim').and.returnValue(of({ body: { claimNumber: 'NEW1' } }));
  ...
  it("saveForm should handle updateClaim error and call progressHide", fakeAsync(() => {
    ...
    spyOn(mockClaims, 'updateClaim').and.returnValue(throwError(() => new Error('fail')));
    const userService = TestBed.inject(UserService) as any;
    spyOn(userService, 'progressHide');
  ...
  it("saveClaim should call postImage and then saveForm when claimNumber exists and files present", fakeAsync(() => {
    ...
    component.ngOnInit();
    component.newClaimForm.controls['replacedWithMohawkMaterial'].setValue(true);
    component.newClaimForm.controls['mohawkReplacementOrder'].setValue('ORD123');
    component.newClaimForm.controls['totalSqFtReplaced'].setValue(10);
    component.newClaimForm.controls['totalLaborRequested'].setValue(100);
  ...
  it("saveClaim should call saveForm when no claimNumber and no files present", fakeAsync(() => {
    ...
    component.ngOnInit();
    component.newClaimForm.controls['replacedWithMohawkMaterial'].setValue(true);
    component.newClaimForm.controls['mohawkReplacementOrder'].setValue('ORD456');
  ...
  it("validateAddressOnFormChange should call productService.validateAddress and set success state when address is valid", fakeAsync(() => {
    ...
    component.newClaimForm.controls['consumeraddressOne'].setValue('123 Main St');
    ...
    spyOn(prod, 'validateAddress').and.returnValue(of({ d: { EvStatus: 'S', EvMessage: 'ok', EsAddress: {} } }));
    ...
    expect(component.validateAddressModal).toHaveBeenCalledWith('Address is valid');
    expect(component.spinnerLoading).toBeFalse();
  }));
  ...
  it("validateAddressOnFormChange should handle service error and call userService.progressHide", fakeAsync(() => {
    ...
    const prod = TestBed.inject(ProductService) as any;
    spyOn(prod, 'validateAddress').and.returnValue(throwError(() => new Error('boom')));
  ...
  it("validateAddressModal should open confirmation modal with provided message", () => {
    ...
    expect(component.openConfirmationModal).toHaveBeenCalledWith(jasmine.objectContaining({
      title: 'Information',
      content: 'Show me',
  ...
  it("validateAddressModal passes empty secondaryActionLabel and action callbacks", () => {
    ...
    const arg = (component.openConfirmationModal as jasmine.Spy).calls.mostRecent().args[0];
