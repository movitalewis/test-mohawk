...
export class ClaimDetailsComponent implements OnInit, OnDestroy {
  ...
  getQueryParamFromUrl() {
    this.router.queryParams.subscribe((params) => {
      ...
      if (claim == "draft") {
        ...
        this.claimNumSub = this.storageService.getItem("claimNumber").subscribe(
          (res) => {
    ...
    this.claimsService
      ...
      .subscribe(
        (res) => {
          ...
          this.claimDetails.invoice = this.setInvoiceItems(
          ...
          this.showPendingWithHeader = this.claimDetails?.invoice?.some(
            (item: any) =>
              item?.approvalPendingWith != undefined &&
              item?.approvalPendingWith != "NA" &&
              item?.approvalPendingWith != "",
          );
          this.claimDetails?.invoice?.map((item: any) => {
            ...
            item.isNonProductLine = !this.checkInvoiceLIne(
              item,
              this.claimDetails?.invoice,
            );
          });
          if (!this.showAuthHeader && !this.commentFlag) {
            if (
              this.columns.some((col: any) => col?.key == "returnAuthNumber")
            ) {
              this.columns.splice(1, 1);
            }
            if (
              this.columns1.some((col: any) => col?.key == "returnAuthNumber")
            ) {
  ...
  claimEdit() {
    ...
    this.claimsService.selectedInvoiceLines.columns = this.columns.filter(
      (item: any) =>
  ...
  deleteClaim() {
    this.claimsService.discardDraft(this.claimNumber).subscribe({
      next: (res) => {
        this.route.navigateByUrl("/residential/claims/history");
      },
      error: (err) => {
        this.userService.progressHide();
        this.route.navigateByUrl("/residential/claims/history");
  ...
  nonProductlength(ln: any) {
    if (ln?.component == "PRODUCT") {
      let a = this.claimDetails?.invoice?.filter(
        (it: any) =>
          it.invoiceSeq == ln.invoiceSeq && it.component != "PRODUCT",
      );
      return a.length;
    }
  }
  ...
  downloadFile(id: any, filename: any) {
    if (id) {
      ...
      this.claimsService.downloadFile(id).subscribe(
        (res: any) => {
  ...
  printPage() {
    ...
    let printSection = this.document.getElementById("print-section");
  ...
  addOrRemoveCssClass(flag: boolean) {
    ...
    [...h1Tags, ...h3Tags, ...h4Tags, ...h5Tags, ...h6Tags, ...pTags].forEach(
      (el: any) => {
        if (flag) {
  ...
  colSpanLength() {
    let value = 0;
    if (
      this.claimDetails?.claimType == this.claimTypes.DEFECTIVE_PRODUCT ||
      ...
      this.claimDetails?.claimType == this.claimTypes.WRONG_QUANTITY_SHORTAGE ||
  ...
  checkInvoiceLIne(line: any, invoices: any) {
    return invoices?.some(
      (item: any) =>
        item.component == "PRODUCT" && line?.invoiceSeq == item?.invoiceSeq,
    );
  }
        ...
        this.creditMemoListService
          ...
          .subscribe(
            (res: any) => {
              ...
              if (!isMobile) {
                a.download =
                  "Invoice-" + selectedListItem?.invoiceNumber + ".pdf";
              } else {
                a.target = "_blank";
              }
              a.click();
  ...
  isOnlyNonProductLine(line: any) {
    return !this.claimDetails?.invoice?.filter(
      (item: any) =>
  ...
  addItemToClaim() {
    this.route.navigateByUrl(
      "residential/claims/add-labor-claim?claim=" + this.claimNumber,
    );
  }

  getInvoiceDetails(invoiceNumber: any) {
    this.claimsService.invoiceDetails(invoiceNumber).subscribe((res: any) => {
      let invoiceLines = (res?.body?.invoicesLines || [])?.flatMap(
        (lines: any) => lines?.invoices || [],
      );
      let claimDetailsLines = (this.claimDetails?.invoice || [])?.filter(
        (line: any) => line?.component != "LABOR",
      );
      if (this.showAllProductLines || this.showProductLines) {
        invoiceLines = invoiceLines?.filter(
          (line: any) => line?.component == "PRODUCT",
        );
        claimDetailsLines = claimDetailsLines?.filter(
          (line: any) => line?.component == "PRODUCT",
        );
      ...
      const isLaborLineAdded =
        (this.claimDetails?.invoice || [])?.filter(
          (line: any) => line?.component == "LABOR",
        )?.length > 0;

      this.claimsService.selectedInvoiceLines.disabledAddItemCTA =
        this.claimLineTypeModalClaims.includes(this.claimType)
          ? this.claimDetails?.laborLineExists == true
            ? isLaborLineAdded
            : false
          : allLinesAdded;
