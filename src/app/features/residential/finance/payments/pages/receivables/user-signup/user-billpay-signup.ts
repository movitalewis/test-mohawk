...
export class UserBillpaySignUpComponent implements OnInit {
  ...
  getSelectBillPayUsers(uid: any) {
    ...
    this.makePaymentService.getSignUpBillPayUsers(uid).subscribe((response: any) => {
  ...
  submitBillPaySignUp() {
    ...
    this.makePaymentService.submitSignUpBillPayUsers(this.selectedEmail).subscribe((response: any) => {
          this.modalService.hide("progressModal");
          this.scrollPageToTop();
          this.messageSuccess = true;
          this.alertData.message = response.body.message;
          this.alertData.type = (response.body.status == 'Failure') ? 'danger' : 'success';
          this.loadingFlag = false;
                  ...
                  .subscribe((res: any) => {
                    if (res?.body) {
                      ...
                      if (!newTab || newTab.closed || typeof newTab.closed === "undefined") {
                        ...
                        this.alertData.message = "Popup blocked! Please allow popups for this site.",
  ...
  handleBillPayClick(): void {
    const selectedUser = this.selectedUsers.find((user: any) => user.customerEmail === this.selectedEmail);
    if (selectedUser) {
  ...
  onTableDataChange(event: any) {
    ...
    this.lastValue =
    this.lastValue > this.selectedUsers.length ? this.selectedUsers.length : this.lastValue;
  }
