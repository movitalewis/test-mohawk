...
export class ProductsCompareComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.designColumns = [
      ...
      { key: "patternRepeat", title: "Pattern Repeat" },
      ...
      { key: "finishedPileThickness", title: "Finsihed Pile Thickness" },
    ];
    ...
    this.warrantColumns = [
      { key: "installMethod", title: "Installation" },
    ];
    ...
    //  res.map((p: any) => {
    //   this.productService.getProductMedias(p.firstVariantCode).subscribe((data: any) => {
    //     p.image= data?.body?.productImageURL+'?$xchangeThumb$'
    //   }, () => {
    //     this.spinnerLoading = false;
    //   });
    //   this.spinnerLoading = false;
  ...
  compareProductDetails(data?: any) {
    ...
    this.productService.compareProducts(`${queryParam}&fields=FULL`).subscribe({
      ...
      this.progressHide()
        this.compareProducts.filter((item: any) => {
          this.productService
            .getProductMedias(item.code)
            .subscribe((data: any) => {
              item.image = data?.body?.productImageURL? data?.body?.productImageURL + "?$xchangeThumb$":'https://s7d4.scene7.com/is/image/MohawkResidential/missing';
            });
          let newKeys: any[] = [];
          for (let k in item) {
            newKeys.push(k);
  ...
  this.progressShow('viewpdf')
    ...
    html2canvas(data, { useCORS: true }).then((canvas) => {
      ...
      pdf.addImage(contentDataURL, "PNG", 0, position + 10, imgWidth, heightLeft);
      let today = new Date();
      pdf.save(`Product-compare-${cValue}.pdf`);
