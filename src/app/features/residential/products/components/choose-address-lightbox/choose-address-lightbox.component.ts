  ...
  implements OnInit
{
  ...
  constructor(
    ...
    this.productCode = this.initialState?.productCode || this.productNumber;
    if (this.initialState !== null && JSON.stringify(this.initialState) !== '{}') {
      ...
      this.isAccessories = this.selectedProduct?.classification == "Accessories" || this.selectedProduct?.productType === 'MERCHANDISING' ? true : false;
      this.showAddtoCart = this.selectedProduct?.classification == "Accessories" || this.selectedProduct?.productType === 'MERCHANDISING' ? true : false;
    }
    if(this.productNumber.includes("#")){
      this.productNumber = this.productNumber.replace(/#/g, '%23');
  ...
  openChangeShippingAddress() {
    const initialState: ModalOptions = {
      initialState: {
        ...
        productCode: this.initialState?.productCode || this.productNumber,
        // Data to  popup
      },
    };
      ...
      this.orderService
        ...
        .subscribe({
          next: (res) => {
            ...
            this.shippingAddress.defaultShipViaDesc = res.body?.defaultShipViaDesc;
    ...
    const initialState: ModalOptions = {
      initialState: {
        ...
        feetyardForm: {
          ...
          originProductType: this.productType || this.initialState?.productType,
  ...
  getShippingAddressMethods() {
    this.ProductService.getShippingMethodWithOutFlag(
      this.reactiveForm.value.zipCode,
      this.reactiveForm.value.oneTimeShippingAddress === undefined
        ? false
  ...
  addTCart() {
    if (this.isCloneOrders) {
      ...
      const payLoad = {
        ...
        //       this.shippingAddress?.incoTerms,
        // shipVia:this.shippingAddress?.defaultShipVia|| this.reactiveForm.value.incoTermsLoc2,
            ...
            requestedQty: this.initialState?.feetyardForm?.requestedQty
              ? this.initialState?.productType === "HARDSURFACE" ||
                (this.initialState.productType === "SOFTSURFACE" &&
                  this.initialState.selectedProduct.subProductType ===
                    "CARPETPRODUCT_CARPET_TILE")
                ? this.initialState?.feetyardForm?.unit === "RO" ? this.initialState?.feetyardForm?.requestedQty
                        : this.initialState?.feetyardForm?.unit === "PK" ?this.initialState?.feetyardForm?.requestedQty
                       :this.initialState?.feetyardForm?.unit === "EA"? this.initialState?.feetyardForm?.requestedQty:this.initialState?.inventoryUOMConvValue ? this.initialState?.inventoryUOMConvValue
                                       : this.initialState?.feetyardForm?.requestedQty
                        : this.initialState?.feetyardForm?.requestedQty
              : this.initialState?.feetyardForm?.feet,
            maxInches: 0,
        ...
        sampleProduct: this.initialState.productType === "MERCHANDISING",
        merchandisingProduct: this.initialState.productType === "MERCHANDISING",
        ...
        shippingInfo: !this.reactiveForm.value.oneTimeShippingAddress
          ...
          : {
              acceptDate: this.reactiveForm?.value.lastestacceptDate
                ...
                : "",
                apptNeeded: this.reactiveForm.value.appoinment === null ? "" : this.reactiveForm.value.appoinment,
                forkLiftRequired: this.reactiveForm.value.accomodate === null ? "" : this.reactiveForm.value.accomodate,
                jobSiteDelivery: this.reactiveForm.value.jobsiteDelivery === null ? "" : this.reactiveForm.value.jobsiteDelivery,
                largestTruckSize: this.reactiveForm.value.truckSize === null ? "" : this.reactiveForm.value.truckSize,
                liftGateAndPallet: this.reactiveForm.value.liftGateAndPallet === null ? "" :  this.reactiveForm.value.liftGateAndPallet,
                loadingDock: this.reactiveForm.value.loading === null ? "" :  this.reactiveForm.value.loading,
                location: this.reactiveForm.value.Location === null ? "" :  this.reactiveForm.value.Location,
                poleLiftRequired: this.reactiveForm.value.poleLift === null ? "" :  this.reactiveForm.value.poleLift,
      ...
      if (this.initialState.productType === "MERCHANDISING") {
        payLoad.orderSamples = [
          {
            code: this.productNumber,
            quantity: this.initialState?.feetyardForm?.requestedQty,
            requestedQty: this.initialState?.feetyardForm?.requestedQty,
            requestedUOM: this.initialState?.feetyardForm?.unit,
            originProductType: this.productType || this.initialState?.productType,
            originSubProductType:  this.initialState?.selectedProduct?.subProductType || "",
            selected: true,
            sellingBackingId:
              this.initialState?.variantData?.selected?.sellingBackingId,
            sellingColorId:
              this.initialState?.variantData?.selected?.sellingColorId,
            sellingColorName:
              this.initialState?.variantData?.selected.sellingColorName,
            sellingSizeId:
              this.initialState?.variantData?.selected?.sellingSizeId,
            sellingStyleId:
              this.initialState?.variantData?.selected?.sellingStyleId,
            size: this.initialState?.variantData?.selected?.sellingStyleId
              ? this.initialState?.variantData?.sizeOptions[0].value?.name
              : "",
            incoTerms:
              this.selectedTab == 2
                ? this.reactiveForm.value.IncoTerms
            ...
            shippingCondition:
              this.selectedTab == 2
                ...
                this.shippingAddress.defaultShippingMethod ||
                  this.shippingAddress.shippingCondition ||
                  ""
               : this.reactiveForm.value.shippingCondition
                : this.userInfo.isCustomer === true || this.userInfo.isSalesPerson === true || this.userInfo.isSalesOps === true ? this.shippingAddress?.originalDefaultShippingMethod || 
                this.shippingAddress.defaultShippingMethod ||
                  this.shippingAddress.shippingCondition ||
                  "":this.shippingAddress.defaultShippingMethod ||
      ...
      this.ProductService.addToCart(
        ...
        next: (res) => {
          if (
            ...
            // if (res.body.cartN umber && cartNumber == null) {
            //   let cartData = {
 ...
 userEnteredAddress() {
          ...
          next: (res) => {
            ...
            if(res?.body?.incoTerms || res?.body?.shipvia){
                ...
                if (res.body?.shippingWarehouse) {
                  ...
                  this.shippingWareHouseSelectedOption = res.body.shippingWarehouse;
                }
                this.incoTermsLoc2Options = [];
                this.incoTermsLoc2Options.push({
                  value: res.body.shipvia || this.shippingAddress?.defaultShipVia,
                  label:
                    res.body.shipViaDesc ||
                    this.shippingAddress?.defaultShipViaDesc,
                });
  ...
  shippingWareHouseModalSubmit() {
    ...
    if (flag === true && (this.userInfo?.isCustomer || this.userInfo?.isSalesPerson  || this.userInfo?.isSalesOps)) {
      this.shippingAddress.defaultShipVia = selectedIncoTermsItem?.label?.toUpperCase() || selectedIncoTermsItem;
      this.shippingAddress.defaultShipViaDesc = selectedIncoTermsItem?.label?.toUpperCase() || selectedIncoTermsItem ;
  ...
  validateShipViaAddress(type: any) {
    ...
    console.log("this.isAtpCheck--->",this.isAtpCheck,"showAddtoCart--->",this.showAddtoCart)
  ...
  populateShippingOptions() {
   ...
   this.incoTermsLoc2SelectedOption?.label ||
    ...
    let shippingWareHouseSelectedOption =
      ...
      this.shippingAddress?.defaultShippingWarehouse;
    let incoTermsSelectedOption =
      this.incoTermsSelectedOption.label ||
      this.incoTermsSelectedOption ||
      this.shippingAddress.defaultIncoTerms;
  ...
  shippingMethodChange() {
    ...
    this.reactiveForm.controls["carrierNumber"].setValue(null);
    this.reactiveForm.controls["IncoTerms"].setValue(null);
    this.reactiveForm.controls["shippingCondition"].setValue(val);
    ...
    this.orderService
      ...
      .subscribe({
        next: (res) => {
          ...
          // this.shippingIncoTermsLoc2Data.push({
          //   value: res?.body[0].shipvia,
          //   label: res?.body[0].shipvia,
          // });
          // }
          const data = res?.body || [];
  ...
  validateShipViaOneTime(event: any) {
    if (event != "" && event != null  && !this.userInfo?.isCustomer && !this.userInfo?.isSalesOps && !this.userInfo?.isSalesPerson) {
      ...
      let shipViaSelectedOption =
        ...
        this.reactiveForm?.value.ShipVia;
      let incoTermsLoc2SelectedOption =
        this.reactiveForm.value?.incoTermsLoc2?.label ||
        this.reactiveForm.value?.incoTermsLoc2 ||
        this.incoTermsLoc2SelectedOption?.label;
      let incoTermsSelectedOption =
        this.reactiveForm.value?.IncoTerms ||
        this.reactiveForm.value?.IncoTerms?.label;
      let shippingWareHouseSelectedOption =
        this.reactiveForm.value?.shippingWareHouse ||
        this.reactiveForm.value?.shippingWareHouse?.label;
