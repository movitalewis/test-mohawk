...
import { SampleBudgetService } from "src/app/features/residential/budget/services/sample-budget.service";
...
export class SampleBudgetComponent implements OnInit {
    ...
    this.sampleBudgetService.transferBudget(this.transferBudget).subscribe(
      ...
      (error) => {
        ...
        const initialState: ModalOptions = {
          initialState: {
            err: true,
            budgetData: error?.error && error?.error?.errors[0].message,
            heading: "Tranfer Error",
            type: "tranfer",
            onClose: () => {
  ...
  ngOnInit(): void {
    ...
    this.districtcolumns = [
      { key: "tmNumber", title: "TM#", orderEnabled: false },
      { key: "salesManName", title: "Name", orderEnabled: false },
      { key: "sampleGroupDesc", title: "Role", orderEnabled: false },
      { key: "sampleGroupDesc", title: "Business Unit", orderEnabled: false },
    ...
    this.samplecolumns = [
      ...
      { key: "po", title: "PO #", orderEnabled: false },
    ...
    this.storageService.getItem("userInfo").pipe(take(1)).subscribe((response: any) => {
      ...
      this.isALCBDM = response?.isALCBDM;
      ...
      this.showCreateBudget = response?.isSalesOps && !response?.isALCBDM;
      this.showBudgetList();
      if (this.showCustomerBudget || this.salesManRole == 'DM') {
        this.getTMNumberList();
      }

      if(response?.isALCBDM){
        this.productTypesArray = this.productTypesArray.filter(
          (item: any) => item.key == "ALC Soft"
        );
      }
    ...
    this.fromTMsuggestions$ = new Observable(
      ...
      switchMap((query: string | undefined) => {
        ...
        if (query && query.length > 1) {
          ...
          return this.sampleBudgetService.getFromTmNumbers(searchText,this.isFutureBuget,this.payload.currency).pipe(
            map((res: any) => {
  ...
  showBudgetList() {
    ...
    this.dmOrderBy = "asc";
    this.tmOrderBy = "asc";
    if (this.showCustomerBudget || this.salesManRole == 'RVP') {
  ...
  pageSizeRVP: number = 10;
  pageSizeDM: number = 10;
  pageSizeTM: number = 10;
  pageSizeSVP: number = 10;
      ...
      .subscribe(
        (res: any) => {
          ...
          let dataObject = {
            ...
            selectedFromName: this.selectedFromName,
  ...
  onSelectFromTM(event: any) {
    if (event.value) {
      this.showFromToError =
        ...
        this.selectedFromToName = `${event.item.tmNumber} | ${event.item.salesManName}`;
      this.transferBudget.fromTmNumber = event.item.tmNumber;
    }
  }
  ...
  allowNumberTwoDecimal(event: any) {
  ...
  onPageSizeChange(e: any, role:any) {
    let value = e?.value;
    let tableItemsSize = Number(value);
    this.setPageSize(role, tableItemsSize);
    this.setPageIndex(role, 0); 
    this.getCustomerBudgetsList(0, role);
  }

  setPageSize(role: any, pageSize: any) {
    switch (role) {
      case "TM":
        this.pageSizeTM = pageSize;
        break;
      case "DM":
        this.pageSizeDM = pageSize;
        break;
      case "RVP":
        this.pageSizeRVP = pageSize;
        break;
      case "SVP":
        this.pageSizeSVP = pageSize;
        break;
    }
  }
}
