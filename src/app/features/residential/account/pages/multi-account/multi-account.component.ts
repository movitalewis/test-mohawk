...
export class MultiAccountComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.mgmtService.getAccountList().subscribe((res) => {
      this.data = res?.body?.filter((account: any) => account.company === "R");
    });
  ...
  onNext(row: any) {
    ...
    const customerAddress =
      row.customerAddress == undefined ? "" : row.addresses[0].formattedAddress;
    ...
    this.userService
      ...
      .subscribe(
        (res) => {
          ...
          this.storageService.setselectedAccount(row?.uid);
          // this.userService.currentUserDetails.next(null);
          // this.getUserInfo();
        },
        (err: any) => {
