...
export class NotificationPreferencesComponent implements OnInit {
  ...
  getUserDetails() {
    this.userService.getCurrentUserDetail().subscribe((response: any) => {
      ...
      //don't show quotes
      // this.setNotifications(this.quoteNotification);
  ...
  //   this.checkAllNotifications(this.quoteNotification, e);
  // }
  ...
  //   this.userService.updateNotifications(this.payload).subscribe({
  //     next: (response) => {
  ...
  onSubmit() {
    ...
    this.userService.updateNotifications(this.payload).pipe(
      switchMap(() => {
      ...
      next: (response) => {
        if (response?.body) {
          ...
          this.userService.profileProgressHide()
          this.alertData = {
            message: response?.body?.message || "My Profile Notifications updated Successfully.",
          };
          this.alertType = "success";
          this.alertTrigger = true;
          this.stopAlert();
