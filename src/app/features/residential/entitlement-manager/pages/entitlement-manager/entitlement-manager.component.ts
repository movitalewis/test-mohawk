import { AfterViewInit, Component, OnInit, HostListener, ElementRef, ViewChild } from "@angular/core";
...
export class EntitlementManagerComponent implements OnInit {
  ...
  initTable() {
    ...
    this.columns = [
      ...
      { key: "styleBlocked", title: "Entitled (Y/N)", width: '1%' },
  ...
  updateCustEntitlement(product:any, operation = '', action = '', from = ''){
    ...
    (err: any) => {
      this.spinnerLoading = false;
      this.alertData = {
        message: 'Entitlement Update Failed',
        type: 'danger',
        showAlert: true
      };
     this.scrollPageToTop();
  ...
  onFilterChange(event: any, type: string){
    ...
    if(type == 'entitled' && event){
      this.payload.entitled = (event == "YES" ? 'Y' : 'N');
  ...
  onCheckboxSeleted(product: any) {
    ...
    // if (this.selectedRows.includes(product.code)) {
    //   this.selectedRows = this.selectedRows.filter((item:any) => item !== product.code);
    //   this.selectedProduct = this.selectedProduct.filter((item:any) => item.code !== product.code);
    // } else {
    //   this.selectedRows.push(product.code);
    //   this.selectedProduct.push(product);
    // }
    // this.allSelected = this.productsData?.length == this.selectedRows?.length;
    
    // this.showBlockedBtn = (this.selectedProduct.filter((item:any) => item.styleBlocked == false).length > 0 ) ? true :  false;
    // this.showUnBlockedBtn = (this.selectedProduct.filter((item:any) => item.styleBlocked == true).length > 0 )? true :  false;
  ...
  //     this.filteredStyles = this.filtersData?.styles.filter((name:string) => name.toLowerCase().includes(value.toLowerCase()));
  //   }
  //   if (!value) {
  //     this.filteredStyles = [];
