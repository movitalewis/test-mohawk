...
export class ReserveDetailsComponent implements OnInit {
  ...
  extendItemModal(allowExtendedData: any) {
    let listData: any = [];
    allowExtendedData.forEach((item: any) => {
      listData.push({
  ...
  checkExtendable() {
    ...
    let allowExtendedData = selectedItems.filter(
      (item: any) => item.isAllowToExtend == true
    );
    let nonExtendedData = selectedItems.filter(
      (item: any) => item.isAllowToExtend == false
    );

    if (nonExtendedData.length > 0 && allowExtendedData.length === 0) {
      const initialState: ModalOptions = {
        initialState: {
    ...
    if (allowExtendedData.length > 0 && nonExtendedData.length === 0) {
      this.extendItemModal(allowExtendedData);
    }
    if (allowExtendedData.length > 0 && nonExtendedData.length > 0) {
      this.openConfirmationModal({
        title: "Confirmation",
        content: `One or more items in your reserve are non-extendable. <br /> Do you want to reserve the eligible items?`,
    ...
    //         })
    //       );
    //       this.bsModalRef.onHide?.subscribe((r: any) =>
