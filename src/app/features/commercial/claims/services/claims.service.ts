...
export class ClaimsService {
  ...
  uploadFile(event: any, component: any) {
    ...
    component.totalFileSize = component.filesArray.reduce((acc: number, f: any) => acc + (f.size || 0), 0);
  ...
  navigateBack() {
     if(this.selectedInvoiceLines.claimNumber  === undefined && !this.claimNumber){
      this.router.navigate(["/residential/claims/history"]);
    }else{
    this.router.navigate(["/commercial/claims/details"], {
      queryParams: {
  ...
  setPhoneNumber(workPhone: any, mobilePhone: any) {
    ...
    return (workPhone && workPhone != undefined && workPhone != "" && workPhone != null) ? workPhone : mobilePhone;
  }
