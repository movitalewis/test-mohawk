...
export class ClaimsHistoryComponent implements OnInit, OnDestroy {
  ...
  navigateToClaimDetail(eventId: any) {
    this.router.navigateByUrl("commercial/claims/details?claim=" + eventId);
  }
selectedFilter: any = [];
accountColumns = false;
  ngOnInit(): void {
    ...
    this.redirectionObj = [
      {
        key: "claimNumber",
        redirectionLink: "commercial/claims/details?claim=",
      },
    ];   
    ...
    this.userInfoSub = this.storageService.getItem("userInfo").pipe(take(1)).subscribe((response: any) => {
      this.salesPersonInHouseFlag = response?.isSalesPerson == true &&
  ...
  getClaimsHistory(pageIndex: any, pageSize: any) {
    let payload : any = {
      salesHierarchyCode: this.salesPersonInHouseFlag ? (this.salesHierarchyCode == undefined ? "" : this.salesHierarchyCode) : "",
      colorName: "",
      colorNumber: "",
      consumerName: "",
  ...
  salesPersonDefaultSelection(tmValue: any) {
    this.salesHierarchyCode = tmValue;
  }
  ngOnDestroy(): void {
