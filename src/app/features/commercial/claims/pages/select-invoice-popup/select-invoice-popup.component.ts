...
import { CLAIM_TYPE_DESC, CLAIM_TYPE_MAP, STANDALONE_NONPROD } from "src/app/features/shared/constants/URL-PERMISSIONS-CONSTANTS";
import { UserService } from "src/app/features/shared/user/services/user.service";
import { CLAIM_PATH_NAMES, LABOR_ELIGIBLE_CLAIMTYPES } from "src/app/features/shared/constants/CLAIMS-CONSTANTS";
@Component({
...
export class SelectInvoicePopupComponent implements OnInit {
        ...
        this.data?.filter((line: any) => {
          line?.invoices?.filter((inv: any) => {
            if (
              ...
              (inv?.component == "CANCELLATION_FEE" || inv?.component == "CALIFORNIA_CARE_FEE")
            ) {
              this.invoiceLines.push(inv);
            } else if (
  ...
  addToClaim() {
    ...
    this.selectedRecords.map((item: any) => {
      item.invoiceTotal = item.invoiceSubTotal?.replace("$", "");
  ...
  selectLineInvoices(e: any, line: any) {
    ...
    this.data.filter((ln: any) => {
      if (ln.invoiceSeq == line.invoiceSeq) {
        ...
        let selectedStandalone = ln?.invoices?.filter((inv: any) => 
          (inv.selected === true && inv?.standaloneLine === true) || (inv.isSelected === true && inv?.standaloneLine === true)
      );
  ...
  disabledCheckbox(row?: any) {
    if (this.claimType == this.claimTypes.FREIGHT) {
      ...
      return (
        row &&
        (row?.component == "FREIGHT" || row?.component == "FUEL_SURCHARGE" || row?.component == "JOB_SITE_DELIVERY" ||
      ...
      this.showNonProductLines = true;
      return (row && 
        (row?.component == "CANCELLATION_FEE" || row?.component =="CALIFORNIA_CARE_FEE")
        );
    } else if (
      this.claimType ==  this.claimTypes.CUSTOMER_SATISFACTION ||
      this.claimType ==  this.claimTypes.MOHAWK_ORDER_ERROR ||
      ...
      return (
        row && (this.nonProdTypes.some((type:any)=> type ==row?.component) || row?.component == "PRODUCT")
      );
    } else if (
      this.claimType === this.claimTypes.PRICING ||
      this.claimType === this.claimTypes.ACCOMMODATION_RETURN
  ...
  checkProductLinesAvailable(){
    ...
    this.data?.forEach((item:any,ind:any) =>{
      if(item?.component !== 'PRODUCT' && item?.invoices?.filter((inv:any) => (inv?.component === 'PRODUCT' && inv?.invoiceSeq === item?.invoiceSeq)).length === 0){
        if(!missedProductLines.includes(item.invoiceSeq)){
      item.invoices.unshift({component : "PRODUCT", invoiceSeq:item.invoiceSeq, isRemovedLine:true, standaloneLine:true})
