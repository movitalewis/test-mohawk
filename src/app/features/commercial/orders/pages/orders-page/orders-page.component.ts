  ...
  implements OnInit, AfterViewInit
{
  ...
  selectTab(tabId: any) {
    ...
    const pageIndices = [this.pageIndex || 1, this.pageIndex2 || 1, this.pageIndex3 || 1];
    const tabActions = [
      {
        name: "Orders",
        ...
        action: () => {
          ...
          this.onOrderSearch(this.payload, pageIndices[0] - 1, this.currentSortCode); 
        },
      },
      {
        ...
        action: () => {
          ...
          this.onShipmentOrders(this.payloadShip, pageIndices[1] - 1, this.shipmentSortCode);
        },
      },
      {
        name: "Samples",
        ...
        action: () => {
          this.payloadShip.salesHierarchyRole = this.salesManRole;
          this.onSampleOrderSearch(this.samplePayload, pageIndices[2] - 1, this.sampleSortCode); 
        },
      },
    ];
    if (tabActions[tabId]) {
      ...
      .subscribe((response: any) => {
        ...
        if (response?.isSalesPerson || response?.isSalesOps) {
          ...
          if (
            ...
            (this.salesManRole == "SVP" || this.salesManRole == "RVP")
          ) {
            this.sampleSearchByList.push({
              id: "sidemark",
              name: "Side Mark",
            });
          }
  ...
  navigateToHistoryDetails(orderId: any, pageNumber: any) {
    ...
    if (this.searchQuery) {
      additionalParams["searchQuery"] = JSON.stringify(this.searchQuery);
    }
    if (pageNumber) {
    ...
    this.router.navigate(
      ["commercial/orders/orders-history-details/" + orderId],
      ...
      .subscribe((res: any) => {
        if (res?.body) {
          ...
          for (let key in res.body) {
            // let name = res?.body[key].replace("_", " ");
  ...
  getSalesList(value: any, orderType: any, role:any) {
    ...
    this.salesHierarchyCode = value;
      if (orderType == "currentOrders") {
        ...
        this.samplePayload.sortOrderBy = "";
        //this.pageIndex3 = 1;
        this.onSampleOrderSearch(this.samplePayload, this.pageIndex-1, this.sampleSortCode);
      }
    // }
  }

  salesPersonDefaultSelection(tmValue: any, role: any) {
    this.payload.salesHierarchyCode = tmValue;
    this.samplePayload.salesHierarchyCode = tmValue;
