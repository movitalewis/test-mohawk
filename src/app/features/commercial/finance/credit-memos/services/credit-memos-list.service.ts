...
export class CreditMemoListService {
  modalRef?: BsModalRef;
  constructor(
  ...
  postCreditMemo(payload: any): Observable<any> {
    return this.apiService.post(API_CONSTANTS.creditMemoSearch.replace(
      "{userId}",
      this.userService.getUserEmail().toLowerCase()
    ), payload);
  }

  getListOfCreditMemos(
    payload: any,
    pageIndex: any,
    orderby: any,
    sortby: any,
    ...
    let url = `${API_CONSTANTS.creditMemoSearch.replace(
      "{userId}",
      this.userService.getUserEmail().toLowerCase()
    )}?fields=DEFAULT&orderby=${orderby}&currentPage=${pageIndex}&sort=${sortby}`;
    url = url.replace("{userId}", this.storageService.userInfo?.uid);
    return this.apiService.post(url, payload);
  }
  getCreditMemoPdf(payload: any): Observable<any> {
