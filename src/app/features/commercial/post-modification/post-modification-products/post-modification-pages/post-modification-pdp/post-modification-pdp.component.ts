...
import { PlpOrderSamplesComponent } from "src/app/features/commercial/products/components/plp-order-samples/plp-order-samples.component";
...
import { OrderService } from "src/app/features/commercial/orders/services/order.service";
...
export class PostModificationPdpComponent implements OnInit {
  ...
  keyPressNumbers(e: any) {
    if (e.currentTarget.value == "") {
      ...
      return (
        /^[0-9]$/i.test(e.key) && Number(e.currentTarget.value + e.key) < 12
      );
    }
  }
  order_number: any;
    ...
    this.orderService
      ...
      .subscribe((res: any) => {
        ...
        if (res && res.body && res.body?.errorCode == "error") {
          ...
          this.shipCompleteFlag = res.body.orderHistoryData[0].shipCompleteOrderFlag;
          }
      });
  }
  subject = new Subject();
  ...
  checkAvailability(type?: any, values?: any) {
    ...
    if (type && type == "Roll") {
      ...
      feetYardFormData.dyeLot = feetYardFormData.dyeLot ? feetYardFormData.dyeLot :'';
    ...
    } else if (this.orderDetails?.sampleOrder === true) {
  ...
  addToCartDirect(item: any, feetyardForm: any, type?: any, value = null) {
    ...
    let payLoad = {
      ...
      lineItems: [
        {
          lineNumber: "",
          dyeLot:feetyardForm.dyeLot,
          requestedQty: feetyardForm.requestedQty,
          requestedUOM: feetyardForm.unit,
          productCode: this.productCode,
    ...
    this.productService.addLineOrAccessories(payLoad).subscribe({
      next: (res) => {
        ...
        if (res?.body?.messages[0].status == "Error") {
          ...
          this.scrollPageToTop();
          this.exceptionErrorMessage = this.isCustomer ? 'Action could not be completed. Sales document is currently being processed.': 'Action could not be completed. '+res?.body?.messages[0]?.message;
        } else {
          this.router.navigate([
            "/commercial/orders/orders-history-details/" +
              this.orderDetails.orderCode,
  ...
  uomDetails: any;
  getUOMDetails() {
    this.service.getUOMDetails(this.productCode).subscribe((res) => {
      ...
      if (res?.body?.alternateUomData) {
        ...
        if (selectedUom.length)
          this.pdpInventoryUomValue = selectedUom[0].alternateUomConversionUnit;
  ...
  getIncoTerms(shipVia: any) {
    ...
    this.orderService.getIncoTerms(shipVia).subscribe({
      next: (res) => {
        ...
        this.incoTermsSelectedOption =
          this.defaultIncoTermsDesc || this.incoTermsOptions[0].label;
      },
      error: (err) => {},
    });
  }
