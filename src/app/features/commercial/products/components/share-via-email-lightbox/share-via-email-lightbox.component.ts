...
export class ShareViaEmailLightboxComponent implements OnInit {
  ...
  emailValidator = (control: FormControl) => {
    ...
    const valid = emails.every((email: any) =>
      this.emailPattern.test(email.trim())
    );
  ...
  sendEmail() {
    ...
    this.progressShow('emailsend', 'emailsendId')
    this.productService.shareViaEmail(payload).subscribe(
      (res: any) => {
        this.progressHide('emailsendId');
  ...
  progressShow(msgType: any, modalId: any = "progressModal") {
