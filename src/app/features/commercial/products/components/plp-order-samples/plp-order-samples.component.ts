import { ChangeDetectorRef, Component, Input, OnInit, OnDestroy, TemplateRef } from "@angular/core";
...
@Component({
    selector: "commercial-plp-order-samples",
...
export class PlpOrderSamplesComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    if (this.initialState?.orderNumber) {
      this.orderService.getOrderDetails(this.initialState?.orderNumber).subscribe((res: any) => {
        this.isShipCompleteOrder = res?.body?.orderHistoryData[0]?.shipCompleteOrderFlag;
      });
    }
  ...
  //   this.serivice.getProductPriceDetails(productCode).subscribe(
  //     (res: any) => {
  ...
  // getProductVariantMatrix(productCode: any, selectedItem: any) {
  //   //selectedItem.sizeOptions = [];
  //   this.spinnerLoading = true;
  //   this.serivice.getPdpVariantRecords(productCode).subscribe(
  //     (res) => {
  //       this.spinnerLoading = false;
  //       if (res.body) {
  //         this.orderSamplesData.filter((el: any) => {
  //           el?.value?.products?.filter((p: any) => {
  //             if (p?.code == productCode) {

  //               selectedItem.sizeOptions = res.body?.sizeOptions;
  //               this.styleName = res.body?.selected?.sellingStyleName;
  //               this.styleId = res.body?.selected?.sellingStyleId;
  //               selectedItem.quantityOptions = this.quantityDropdownData;
  //               selectedItem.size =
  //                 selectedItem.size === undefined || selectedItem.size == ""
  //                   ? res.body?.sizeOptions[0].key
  //                   : selectedItem.size;
  ...
  addToCartClick(products: any) {
    ...
    mockcartRequest = {
      ...
      replacementOrderNumber: this.shippingAddress?.orderNumber
        ...
        : "",
      pdpProductCode: this.initialState?.productCode,
      phoneNumber: "",
  ...
  disableAddtoCart(samples: any, key: any) {
    ...
    return (key == "Architect Folders" ? (data?.length == 0) : (selectedRecords.length == 0 || (selectedRecords.length && qtyFlag)));
    // return (
