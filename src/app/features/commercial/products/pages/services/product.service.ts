...
export class ProductService {
  ...
  allAccessoryDetails(type: string, productCode: string): Observable<any> {
    let url = API_CONSTANTS.allAccessory;
    url = url.replace("{productId}", productCode);
    return this.apiService
      .getUsingDynamicCatalog(`${url}${type ? `/${type}` : ""}`)
      .pipe(catchError((error: HttpErrorResponse) => of(error)));
  }
  ...
  cartData() {
    this.createCart().subscribe((res) => {
      let data = res.body;
      data.code = res.cartNumber;
      this.storageService.setItem("miniCartCount", data);
    });
  }
  addToCart(userId: string, cartId: any, payLoad: any) {
  ...
  getQuoteVariantColor(code: any): Observable<any> {
    const url = API_CONSTANTS.colorVariants
  ...
  placeReserve(cartId: any, resreserveName: string): Observable<any> {
    let url = API_CONSTANTS.placeReserve.replace("{cartId}", cartId);
    url = url.replace("{reserveName}", encodeURIComponent(resreserveName));
    return this.apiService
      .post(url, {})
      .pipe(catchError((error: HttpErrorResponse) => of(error)));
  }
  ...
  getDivision(cartId?: any, data?: any) {
    let url = API_CONSTANTS.getDivision
      .replace("{userId}", this.userService.getUserEmail().toLowerCase())
      .replace("{cartId}", cartId);
    url = url.replace("{cartId}", "cartId");
  ...
  getSubDivision(cartId?: any, data?: any) {
    let url = API_CONSTANTS.getSubDivision
      .replace("{userId}", this.userService.getUserEmail().toLowerCase())
      .replace("{cartId}", cartId);
    url = url.replace("{cartId}", "cartId");
    return this.apiService.post(url, data);
  ...
  postImage(filesArray: any, url: any) {
    ...
    filesArray.forEach((item: any) => {
      formData.append(
        "file",
        new File([item.file], item.name, { type: "application/octet-stream" })
      );
    });
    return this.apiService
      .postFile(
  ...
  postImageForCart(filesArray: any, url: any) {
    const formData = new FormData();
    filesArray.forEach((item: any) => {
      formData.append(
        "file",
  ...
  getPlpPriceSearch(payload: any, soldTo:any) {
  ...
  getCreList(uid: any, search: any): Observable<any> {
    let url = API_CONSTANTS.creList
      .replace("{uid}", uid)
      .replace("{search}", search);
  ...
  getGpoList(uid: any, search: any): Observable<any> {
    let url = API_CONSTANTS.gpoList
      .replace("{uid}", uid)
      .replace("{search}", search);
  ...
  getAdList(uid: any, search: any): Observable<any> {
    let url = API_CONSTANTS.adList
      .replace("{uid}", uid)
      .replace("{search}", search);
  ...
  getBundledReferences(productCode:any){
    let url = API_CONSTANTS.getBundledReferences.replace("{productCode}", productCode);
     return this.apiService.get(url);
  }
