...
export class AccountSearchComponent implements OnInit, OnDestroy {
  ...
  zipBaseLength: any = 0;
  quoteBaseSort: any = "";
  quoteBaseOrderBy: any = "desc";
  quoteBaseLength: any = 0;
  ...
  checkForPassKeyRequired(row: any) {
    ...
    if (row?.passKeyProtected) {
      ...
      this.spinnerLoading = false;
    } else if(row?.companies?.length > 1){
  ...
  onNext(row: any) {
    ...
    this.userService
      ...
      .subscribe(
        (res) => {
          // if (this.userService.currentUserDetails.getValue().body?.defaultStorefront == "C") { 
  ...
  selectTab(tabId: any) {
    if ((this.tabId === 0 && tabId !== 0) || (this.tabId === 5 && tabId !== 5)) {
  ...
  onSearch(data: any, template3: TemplateRef<any>) {
    ...
    }us_b2b_commercial/users/${this.userService.getUserEmail().toLowerCase()}/csrcustomer/search?currentPage=${currentPage}&fields=DEFAULT&pageSize=${pageSize}&showMode=Page&orderby=${orderBy}&sort=${sort}`;
    // this.spinnerLoading = true;
    if (data) {
  ...
  getDataByQuoteNumber(url: any): Observable<any> {
  ...
  checkQuoteValue() {
    this.quoteForm.get("quoteNumber")?.valueChanges.subscribe((value) => {
      for (let i = 0; i < value.length; i++) {
        if (!(parseInt(value[i]) >= 0 && parseInt(value[i]) <= 9)) {
          this.quoteForm.get("quoteNumber")?.setValue("");
          break;
        }
      }
    });
  ...
  onQuoteSearch(data: any, template3: TemplateRef<any>) {
    // sessionStorage.setItem("tabId", this.tabId.toString());
    this.errorMessage = "Please provide a valid quote number.";
    // let currentPage = this.pageIndex - 1;
    // let pageSize = this.tableItemsSize;
    // let orderBy = this.customerBaseOrderBy;
    // let sort = this.customerBaseSort;
    this.quoteBaseCol = [];
    let url = `${environment.baseAPIURl}us_b2b_commercial/users/${this.userService.getUserEmail().toLowerCase()}/quotes/quoteCSRSearch/${data.quoteNumber}`;
    // this.spinnerLoading = true;
    this.quoteBaseData = [];
    if (data) {
      this.accountService.progressShow('accountSearch');
      this.getDataByQuoteNumber(url).pipe(take(1)).subscribe({
        next: (res) => {
          this.accountService.progressHide();
          console.log('res',res);
          ...
          this.spinnerLoading = false;
          if (res?.errorCode) {
            this.errorMessage = res?.message || this.errorMessage;
            this.bsModalRef = this.modalService.show(template3, {
              id: "onSearch-template-3",
              class: "modal-md modal-dialog-centered",
              backdrop: "static",
            ...
            // if (res?.csrQuoteSearchResponseList?.hasOwnProperty("quoteCode")) {             
            //   this.quoteBaseCol = [{
            //     key: "quoteCode",
            //     title: "Quote Number",
            ...
            //   }];
            // }else if(res?.csrQuoteSearchResponseList?.hasOwnProperty("orderCode")){
            //   this.quoteBaseCol = [{
            //     key: "orderCode",
            //     title: "Order Number",
            ...
            //     }];
            // }
            // res.customerNumber = res?.b2bUnitId;
         
            // this.quoteBaseData.push(res);
            // this.quoteBaseLength = this.quoteBaseData?.length || 0;
            if (res?.csrQuoteSearchResponseList?.length) {
              const responseItem = res.csrQuoteSearchResponseList[0]; // Get the first item from the list
            
              // Dynamically set columns based on the presence of keys
              if (responseItem.hasOwnProperty("quoteCode")) {
                this.quoteBaseCol = [
                  { key: "quoteCode", title: "Quote Number" },
                  { key: "customerNumber", title: "Account Number" }
                ];
              } else if (responseItem.hasOwnProperty("orderCode")) {
                this.quoteBaseCol = [
                  { key: "orderCode", title: "Order Number" },
                  { key: "b2bUnitId", title: "Account Number" }
                ];
              }
            
              res.csrQuoteSearchResponseList.forEach((item: any) => {
                const processedItem = {
                  ...item,
                  customerNumber: item.b2bUnitId,
                  // Assign quoteCode only if it exists, otherwise leave undefined
                  quoteCode: item.quoteCode ? item.quoteCode : undefined
                };
                this.quoteBaseData.push(processedItem);
              });
            }
  ...
  navigateToQuoteDetails(row: any) {    
    this.payload = {
      customerNumber: this.displayCustNumber(row.customerNumber),
      accountType: "C",
    };

    let url = `${environment.baseAPIURl}rest/v2/us_b2b_commercial/users/${this.userService.getUserEmail().toLowerCase()}/csrcustomer/search?currentPage=0&fields=DEFAULT&pageSize=10&showMode=Page&orderby=desc&sort=`;
    
    if (row?.quoteCode) {
      this.accountService.progressShow('navigateToQuote');
    } else if (row?.orderCode) {
      this.accountService.progressShow('navigateToOrder');
    }
    this.getUser(url, this.payload).subscribe(
      (result: any) => {
        this.spinnerLoading = false;
        let data = result?.soldTo[0];
        localStorage.setItem("accountNumber", data.customerNumber);
        localStorage.setItem("customerName", data.accountName);
        const addressLine2 = data.addressLine2 == undefined ? "" : data.addressLine2;
        localStorage.setItem(
          "customerAddress",
          addressLine2 +
          " " +
        ...
        this.storageService.setItem("uid", data?.customerNumber);
        this.storageService.uidForDuplicate = row?.customerNumber;
        // this.spinnerLoading = true;
        this.userService
          .setUnit("?unitUid=" + localStorage.getItem("accountNumber"))
          .subscribe(
            (res) => {
              ...
              this.spinnerLoading = false;
              if (row?.quoteCode) {
                this.router.navigate([`/commercial/quotes/quote-detail/${row.quoteCode}`]);
              } else if (row?.orderCode) {
                this.router.navigate([`/commercial/orders/orders-history-details/${row.orderCode}`]);
              }
